...
<!--    <a class="layui-btn layui-btn-warm layui-btn-xs layui-btn-radius" lay-event="selectCarRole">分配角色</a>-->
<!--    <a class="layui-btn layui-btn-normal layui-btn-xs layui-btn-radius" lay-event="resetCarPwd">重置密码</a>-->
...
<%--用户分配角色的弹出层开始--%>
<div style="display: none;padding: 10px" id="selectCarRole">
...
<script type="text/javascript">
    ...
    layui.extend({
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.id + '】这个用户么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${yeqifu}/car/deleteCar.action", {carid: data.carid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
                ...
                //编辑，打开修改界面
                openUpdateCar(data);
            }else if(layEvent === 'selectCarRole'){//分配权限
                openselectCarRole(data);
            }else if(layEvent === 'resetCarPwd'){//重置密码
                layer.confirm('真的重置【' + data.id + '】这个用户的密码吗？', function (index) {
                    //向服务端发送重置密码
                    $.post("${yeqifu}/car/resetCarPwd.action", {carid: data.carid}, function (res) {
                        layer.msg(res.msg);
                    })
                });
            }
        ...
        //打开分配角色的弹出层
        function openselectCarRole(data) {
            mainIndex=layer.open({
                type:1,
                title:'给【'+data.id+'】分配角色',
                content:$("#selectCarRole"),
                area:['700px','390px'],
                btnAlign:'c',
                ...
                yes:function (index, layero) {
                    ...
                    //保存
                    $.post("${yeqifu}/car/saveCarRole.action",params,function (obj) {
                        layer.msg(obj.msg);
                        //关闭弹出层
                        layer.close(mainIndex);
                    })
                ...
                success:function (index) {
                    ...
                    var roleTableIns = table.render({
                        ...
                        , url: '${yeqifu}/car/initCarRole.action?carid='+data.carid //数据接口
                        , title: '角色列表'//数据导出来的标题
                        , cellMinWidth: 100 //设置列的最小默认宽度
                        , cols: [[   //列表数据
                            {type: 'checkbox', fixed: 'left'}
